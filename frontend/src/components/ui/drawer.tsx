...
export const Drawer = forwardRef<HTMLDivElement, DrawerProps>(
  (
    ...
    const sizeClasses = {
      sm: side === "bottom" ? "h-[30dvh]" : "max-w-xs",
      md: side === "bottom" ? "h-[50dvh]" : "max-w-md",
      lg: side === "bottom" ? "h-[85dvh]" : "max-w-xl",
    };

    const sideClasses = {
      left: "left-0 top-0 bottom-0 border-r animate-drawer-left",
      right: "right-0 top-0 bottom-0 border-l animate-drawer-right",
      bottom: "bottom-0 left-0 right-0 border-t rounded-t-[var(--radius-xl)] pb-[env(safe-area-inset-bottom)] animate-drawer-bottom",
    };

    return (
      <Portal>
        <div className="fixed inset-0 z-50 flex bg-black/60 backdrop-blur-sm animate-overlay-fade">
          <div
          ...
          <div
            ...
            tabIndex={-1}
            className={cn(
              "absolute flex flex-col overflow-hidden bg-[var(--surface-elevated)] border-[var(--border)] shadow-2xl",
              side !== "bottom" && "w-full h-full",
              side === "bottom" && "w-full mt-auto",
              sizeClasses[size],
              sideClasses[side],
              className
            )}
