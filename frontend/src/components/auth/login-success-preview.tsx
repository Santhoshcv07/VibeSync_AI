...
export interface LoginSuccessPreviewProps {
  onReset: () => void;
}

export function LoginSuccessPreview({ onReset }: LoginSuccessPreviewProps) {
  ...
  return (
    <Card className="w-full p-8 text-center flex flex-col items-center animate-fade-in bg-[var(--surface-elevated)] border-[var(--border)]">
      ...
      >
        Prototype sign-in preview complete
      </h2>
      
      <p className="text-body text-foreground-muted mb-8 max-w-sm mx-auto">
        Your login details passed local validation. No credentials were verified, no session was created, and no information was sent or stored.
      </p>
