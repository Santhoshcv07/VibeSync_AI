          ...
          return (
            <div key={item.id} className="flex flex-col h-full">
              <a 
                ...
                className="relative aspect-[2/3] w-full rounded-md overflow-hidden bg-[#1c0f35] border border-[#291245] block mb-3 group shadow-md"
              >
                {item.imageUrl ? (
                  <img src={item.imageUrl} alt={item.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                ) : (
                  <div className="w-full h-full bg-gradient-to-br from-[#fef3c7] to-[#d97706] opacity-90 flex flex-col items-center justify-center p-4 text-center">
              ...
              <div className="flex flex-col flex-1">
                ...
                <p className="text-[10px] text-white/50 truncate mb-3">{item.creator}</p>
