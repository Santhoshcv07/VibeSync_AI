import { ProfileMood, profileMoodOptions } from "./profile.data";
import { cn } from "@/lib/cn";

export interface ProfileMoodSelectorProps {
  value: ProfileMood;
  onChange: (value: ProfileMood) => void;
}

export function ProfileMoodSelector({ value, onChange }: ProfileMoodSelectorProps) {
  return (
    <fieldset className="space-y-4">
      <div className="space-y-1">
        ...
        <p className="text-sm text-zinc-400">
          Choose the emotional style you return to most often.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        {profileMoodOptions.map((option) => {
          ...
          return (
            ...
            >
              <input
                ...
                aria-describedby={`mood-desc-${option.value}`}
              />
